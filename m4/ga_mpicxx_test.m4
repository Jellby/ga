# GA_MPICXX_TEST_PROGRAM
# ---------------------
# Create an MPI test program in C++.
AC_DEFUN([GA_MPICXX_TEST_PROGRAM], [
AC_LANG_PUSH([C++])
...
AC_DEFUN([GA_MPICXX_TEST_LINK], [
AC_LANG_PUSH([C++])
GA_MPICXX_TEST_PROGRAM()
ga_cv_cxx_mpi_test_link=no
AS_IF([test "x$ga_cv_cxx_mpi_test_link" = xno],
    [AC_MSG_CHECKING([whether a C++ MPI program links natively])
     AC_LINK_IFELSE([],
        [ga_cv_cxx_mpi_test_link=yes
         GA_MP_LIBS=
         GA_MP_LDFLAGS=
         GA_MP_CPPFLAGS=],
        [ga_cv_cxx_mpi_test_link=no])
     AC_MSG_RESULT([$ga_cv_cxx_mpi_test_link])])
...
AS_IF([test "x$ga_cv_cxx_mpi_test_link" = xno],
    [AC_MSG_CHECKING([for mvapich libraries])
     ...
     do
        LIBS="$LIBS $lib"
        AC_LINK_IFELSE([],
            [ga_cv_cxx_mpi_test_link="$lib"; break],
            [ga_cv_cxx_mpi_test_link=no])
        LIBS="$ga_save_LIBS"
     done
     LIBS="$ga_save_LIBS"
...
CPPFLAGS="$ga_save_CPPFLAGS"
AC_LANG_POP([C++])
AS_CASE([$ga_cv_cxx_mpi_test_link],
    [yes],  [],
    [no],   [AC_MSG_FAILURE([could not link a C++ MPI program])],
    [*],    [GA_MP_LIBS="$ga_cv_cxx_mpi_test_link"],
    [])
])dnl
